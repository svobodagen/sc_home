...
import { View, StyleSheet, FlatList, Pressable, ScrollView, Image, Text, Modal, TextInput } from "react-native";
...
export default function HostProjectsScreen() {
    ...
    const loadData = async () => {
        try {
            ...
            console.error("Chyba při načítání projektů pro hosta:", error);
        } finally {
            setLoading(false);
        }
    };
    ...
    const getSelectedApprenticeName = () => {
        if (!selectedApprentice) return "Všichni učedníci";
        const app = apprentices.find(a => a.id === selectedApprentice);
        return app?.name || "Vybrat učedníka";
    };

    const getSelectedMasterName = () => {
        if (!selectedMaster) return "Všichni mistři";
        const master = masters.find(m => m.id === selectedMaster);
        return master?.name || "Vybrat mistra";
    };
        ...
        return (
            <View style={styles.projectGridItem}>
                <ProjectCard
                    ...
                    onPress={() => navigation.navigate("ProjectDetail", {
                        project: item,
                        projectIndex: index, // Pass the correct index from the filtered list
                        apprenticeId: item.user_id,
                        isGlobal: true,
                        projectList: filteredProjects // Pass the current filtered list
                    })}
    ...
    const renderFilters = () => (
        <View style={styles.filtersContainer}>
            <View style={styles.titleRow}>
                <ThemedText style={styles.sectionTitle}>Projekty Učedníků</ThemedText>
    ...
    const renderSelectionModal = () => {
        ...
        // Filter logic: show only connected users if the other filter is active
                            ...
                            <FlatList
                                ...
                                renderItem={({ item }) => {
                                    ...
                                    if (isApprentice) {
                                        ...
                                        // Item is Master (Master ID is in master_id column)
